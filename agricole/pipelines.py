...
class AgricolePipeline:

    # Database setup
    conn = sqlite3.connect('agricole.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `agricole`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM agricole WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO agricole (date, title, link, content)"
